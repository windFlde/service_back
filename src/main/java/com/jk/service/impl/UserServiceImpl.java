...
public class UserServiceImpl implements UserService
{
    ...
    private void getTree(List<Integer> permissionIds, List<Tree> list) {
        ...
        for (Tree tree : list) {
            List<Tree> date = userMapper.queryPermission(tree.getId());
            ...
            if (permissionIds != null) {
                ...
                boolean exisChildren = tree.getChildren() != null && tree.getChildren().size() > 0;

                // 如果当前节点有子节点就不能为勾选状态 如果子节点中有未勾选的节点 父节点如果选中择所有子节点全为勾选状态
                if (permissionIds.contains(tree.getId()) && !exisChildren) {//
                    // 如果当前遍历的树在 permissionIds集合中则当前树应该是勾选状态

                    // 如果当前遍历的树在 permissionIds集合中 并且当前节点没有子节点
                    tree.setChecked(true); // 就把其设置为勾选状态
                }
